...
class Neural:
	def __init__(self, activation='sigmoid', input_size=2, learning_rate=0.1, epochs=1000):
		...
		hidden_layer_count1 = 10  # Количество нейронов первого скрытого слоя
		hidden_layer_count2 = 5  # Количество нейронов второго скрытого слоя
		
		# Скрытый слой 1
		self.W1 = tf.Variable(
			tf.random.normal([input_size, hidden_layer_count1], stddev=np.sqrt(2/input_size)), dtype=tf.float32)
		self.b1 = tf.Variable(tf.fill([hidden_layer_count1], -1.0), dtype=tf.float32)

		# Скрытый слой 2
		self.W2 = tf.Variable(tf.random.normal([hidden_layer_count1, hidden_layer_count2], stddev=np.sqrt(2/hidden_layer_count1)), dtype=tf.float32)
		self.b2 = tf.Variable(tf.fill([hidden_layer_count2], -1.0), dtype=tf.float32)

		# Выходной слой 1 нейрон
		self.Wo = tf.Variable(tf.random.normal([hidden_layer_count2, 1], stddev=np.sqrt(2/hidden_layer_count2)), dtype=tf.float32)
	...
	def train(self, X, Y):
		...
		for epoch in range(self.epochs):
			...
			with tf.GradientTape() as tape:
				...
				# Выходной слой
				output_layer = 1 / (1 + tf.exp(-(tf.matmul(hidden_layer2, self.Wo) + self.bo)))
